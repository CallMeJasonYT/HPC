...
float weno_omp_core(const float a, const float b, const float c, const float d, const float e)
{
...
void weno_omp_reference(const float * const a, const float * const b, const float * const c,
                          ...
                          const int NENTRIES)
{
    ...
    {
        out[i] = weno_omp_core(a[i], b[i], c[i], d[i], e[i]);
    }
}
